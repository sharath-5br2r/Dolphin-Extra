...
!define PRODUCT_NAME "Better Wii Menu DE"
!define PRODUCT_PUBLISHER "Better Wii Menu DE"
!define PRODUCT_WEB_SITE "https://github.com/Gavin-S-Dev/dolphinfork"
...
; MUI Settings
; TODO: Replace with custom Better Wii Menu DE icon
...
Section "Base"
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\$DisplayName.lnk" "$INSTDIR\BetterWiiMenuDE.exe"
  CreateShortCut "$DESKTOP\$DisplayName.lnk" "$INSTDIR\BetterWiiMenuDE.exe"
...
Section -Post
  ...
  WriteRegStr SHCTX "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\BetterWiiMenuDE.exe"
  ...
  WriteRegStr SHCTX "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\BetterWiiMenuDE.exe"
  ...
  WriteRegStr SHCTX "${PRODUCT_UNINST_KEY}" "Comments" "Better Wii Menu DE - Enhanced Wii Menu with disc image channel support"
SectionEnd
...
Section Uninstall
  ...
  Delete "$INSTDIR\BetterWiiMenuDE.exe"
